...
use super::base_meter::{INPUT_ICONS, MAX_NATURAL_VOL, MAX_SCALE_VOL};
...
pub struct SourceMeter {
    ...
    l_id: RefCell<Option<glib::signal::SignalHandlerId>>,
    s_id: RefCell<Option<glib::signal::SignalHandlerId>>,
}
...
impl SourceMeter {
    ...
    pub fn new(pulse: Shared<Pulse>) -> Self {
        ...
        Self {
            ...
            split: false,
            peak: None,
            l_id: RefCell::new(None),
            s_id: RefCell::new(None),
        }
    }
    ...
    fn rebuild_widgets(&mut self) {
        ...
        if let Some(id) = self.s_id.borrow_mut().take() {
        ...
        if let Some(id) = self.l_id.borrow_mut().take() {
            self.widgets.app_button.disconnect(id)
        }
        self.l_id
            .replace(Some(self.widgets.app_button.connect_clicked(
                move |trigger| {
    ...
    fn show_popup(trigger: &gtk::Button, pulse_shr: &Shared<Pulse>, index: u32) {
        ...
        set_default.set_property("role", gtk::ButtonRole::Check);
        set_default.set_property("text", Some("Set as Default"));
        set_default.set_property("active", pulse.default_source == index);
        ...
        set_default.connect_clicked(move |set_default| {
            ...
            set_default.set_property("active", true);
        ...
        if pulse.sources.len() >= 2 {
            ...
            for (i, v) in &pulse.sources {
                ...
                button.set_property("role", gtk::ButtonRole::Radio);
                button.set_property("active", v.data.index == index);
