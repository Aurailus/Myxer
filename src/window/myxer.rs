...
impl Myxer {
    ...
    pub fn new(app: &gtk::Application, pulse: &Shared<Pulse>) -> Self {
...

        {
            ...
            window.style_context().add_class("Myxer");
            style::style(&window);
            ...
            let icon = gtk::Image::from_icon_name(
                Some("multimedia-volume-control"),
                gtk::IconSize::Button,
            );
...

        {
            ...
            prefs_button.style_context().add_class("flat");
            ...
            show_visualizers.set_property("text", Some("Visualize Peaks"));
            ...
            about.set_property("text", Some("About Myxer"));
...

        {
            ...
            output_scroller.style_context().add_class("bordered");
            ...
            input_scroller.style_context().add_class("bordered");
    ...
    pub fn update(&mut self) {
        ...
        if self.pulse.borrow_mut().update() {
            ...
            let offset = meters.sink.widget.allocation().height
                + meters.sink.widget.margin_bottom()
                - meters.sink_inputs_box.allocation().height;
            if offset != meters.sink.widget.margin_bottom() {
                meters.sink.widget.set_margin_bottom(offset)
            }

            let offset = meters.source.widget.allocation().height
                + meters.source.widget.margin_bottom()
                - meters.source_outputs_box.allocation().height;
            if offset != meters.source.widget.margin_bottom() {
            ...
            for (index, input) in &pulse.sink_inputs {
                ...
                if meter.widget.parent().is_none() {
